...
/*
 * Game Start, you will put the first chess. (You are white because a opening with first three chesses are put by system)
...
struct Position aiTurn(const char board[BOARD_SIZE][BOARD_SIZE], int me, int otherX, int otherY) {
    ...
    if (turn == 1 && EMPTY == board[2][0]) {
      preferedPos.x = 2;
      preferedPos.y = 0;
      turn++;
    } else if (turn == 2 && EMPTY == board[0][1]) {
      preferedPos.x = 0;
      preferedPos.y = 1;
      turn++;
    } else if (turn == 3 && EMPTY == board[0][2]) {
      preferedPos.x = 0;
      preferedPos.y = 2;
      turn++;
    } else if (turn == 4 && EMPTY == board[0][0]) {
      //A forbidden move
      preferedPos.x = 0;
      preferedPos.y = 0;
      turn++;
